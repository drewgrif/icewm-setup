...
# JustAGuy Linux - IceWM Setup
# https://github.com/drewgrif/icewm-setup
...
CONFIG_DIR="$HOME/.icewm"
TEMP_DIR="/tmp/icewm_$$"
LOG_FILE="$HOME/icewm-install.log"
...
export_packages() {
    echo "=== IceWM Setup - Package Lists for Different Distributions ==="
    echo
    ...
    local arch_packages=(
        ...
        "base-devel icewm sxhkd xdotool"
        "libnotify"
    ...
    local fedora_packages=(
        ...
        "gcc make git icewm sxhkd xdotool"
        "libnotify"
    ...
    echo "  $0 --only-config    # To copy just the IceWM configuration files"
}
...
echo " +-+-+-+-+-+-+-+-+-+-+-+-+-+ "
echo " |i|c|e|w|m| |s|e|t|u|p|    | "
echo " +-+-+-+-+-+-+-+-+-+-+-+-+-+ "
echo -e "${NC}\n"

read -p "Install IceWM? (y/n) " -n 1 -r
echo
[[ ! $REPLY =~ ^[Yy]$ ]] && exit 1
...
PACKAGES_CORE=(
    ...
    build-essential icewm sxhkd xdotool
...
if [ -d "$CONFIG_DIR" ]; then
    clear
    read -p "Found existing IceWM config. Backup? (y/n) " -n 1 -r
    echo
    if [[ $REPLY =~ ^[Yy]$ ]]; then
...
cp -r "$SCRIPT_DIR"/.icewm/* "$CONFIG_DIR"/ || die "Failed to copy IceWM config"

# Configuration directories are already in the .icewm folder, so we don't need to copy them separately
...
echo "1. Log out and select 'IceWM' from your display manager"
